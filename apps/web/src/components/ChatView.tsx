...
export default function ChatView(props: ChatViewProps) {
  ...
  const restoreQueuedMessagesToComposer = (messages: ReadonlyArray<QueuedComposerMessage>) => {
    ...
    if (overflow.length > 0 && activeThreadKey) {
      useQueuedMessageStore.getState().enqueue(activeThreadKey, {
        ...
        ...(messages[0]?.sendSettings ? { sendSettings: messages[0].sendSettings } : {}),
  ...
  const onSend = async (
    ...
    if (!sendCtx || (!queuedMessage && !sendCtx.providerAvailable)) {
      notifyDirectAnnotationAttached();
      return;
    }
    ...
    const sendSettings = queuedMessage?.sendSettings ?? {
  ...
  // Sends the oldest queued message after the active turn ends. Starting that
  // send moves the phase out of ready, so only one message leaves per turn.
          ...
          >
            ...
            <div className="relative flex min-h-0 flex-1 flex-col bg-background">
              ...
              <MessagesTimeline
                ...
                {...(!paintOnlyDisplayedTimeline && supportsForking ? { onForkUserMessage } : {})}
