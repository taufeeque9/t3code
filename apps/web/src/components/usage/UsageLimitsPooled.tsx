...
import { ClaudeSignIns } from "./ClaudeSignIns";
import { extraUsageSummary } from "./extraUsage";
import {
  PaceIcon,
  ResetCreditDialog,
  barColor,
  ...
  return (
    <div className="flex w-72 max-w-[calc(100vw-3rem)] flex-col gap-2.5 text-xs">
      ...
      <div className="flex flex-col gap-1 border-t border-border/60 pt-2.5">
        ...
        {account.limits.extraUsage ? (
          <Row label="Extra">{extraUsageSummary(account.limits.extraUsage)}</Row>
        ) : null}
      </div>
      {credits && redeem ? (
...
export function UsageLimitsPooled({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ClaudeSignIns presentations={presentations} />
    </div>
  );
}
