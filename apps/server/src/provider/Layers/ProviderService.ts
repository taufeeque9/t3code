    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (changedInstanceWithinProvider) {
          // An instance the registry no longer knows reports its own error;
          // masking it here would call a missing provider a validation fault.
          const persistedInstanceInfo = yield* registry.getInstanceInfo(persistedInstanceId);
          const continuationIdentityMatches =
            persistedInstanceInfo.driverKind === resolvedProvider &&
