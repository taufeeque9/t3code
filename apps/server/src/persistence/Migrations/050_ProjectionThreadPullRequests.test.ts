...
layer("050_ProjectionThreadPullRequests", (it) => {
  it.effect("upgrades an existing custom search database and backfills legacy single links", () =>
    Effect.gen(function* () {
      ...
      const searchTables =
        yield* sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'projection_thread_search_units'`;
      assert.equal(searchTables.length, 1);
